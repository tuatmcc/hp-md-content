...
一通りTypstを用いてレポートを完成させることができるようになることが今回のゴールです!
# Typstとは
- いわゆる組版エンジン
...
大事なのは構成をきれいにまとめること。そうでないと認知負荷が高くなってしまい、レポートを読む人が内容を理解しづらくなってしまいます。

Wordは構造的にレポートを書くのに向いているとは言えません。例えば章を書くときに、ただ文字サイズを変えて太字にしているだけだと、構造がわかりづらくなってしまうし、図や表に番号をそのまま書いていると、途中で図を挿入し直したときに番号を振り直す必要が出てきてしまいます。

Typstを用いると、構造を自然に意識して書くことができるようになり、図や表の番号も自動的に振られるようになるので、レポートを書く上での認知負荷を減らすことができます。
...
インストーラを起動したらインストールウィザードの案内どおりに入れる。「VSCodeで開く」を追加するオプションは入れておいたほうが良いです。
...
## テンプレートをクローンする
今回はレポートのテンプレートを用意したりしているので、このリポジトリをクローンします。
```sh
...
タブバーあたりにPreview, PDFのアイコンがあります。
Previewをクリックすると右側にプレビューが表示されます。
PDFをクリックすると同階層にPDFが生成されます。

![Preview, PDFアイコン](./img/preview.png)

# 基本的な文法
ここではレポートを書く上で最低限必要な文法を見ていきます。
ここから実際に自分自身で実際に`report.typ`へ記述してどのように表示されるかを実際に確認していきましょう！
## 章立て
「=」で章立できます。
深さは「=」の数で表されます。
```
...
例えば次のように書くと、インライン数式が表示されます。

文字が連続する場合はスペースを入れる必要があることに注意してください。
```
この数式は $E=m c^2$ として表される。
```
...
生成された画像は
![save image](./img/save-img.png)
にある保存の`Save as`ボタンをクリックして任意の場所に保存できます。
あとは図の挿入のところで説明した方法でTypstのレポートに挿入してみましょう！
...
時間に余裕があって希望者がいればGitHubアカウントを作成して、リポジトリを作成して、コミット・プッシュでコードを管理するところも説明できるかも？

Git/GitHubについては[先輩が残した素晴らしい資料](https://tuatmcc.com/workshops/gitlec-step0-2025/chapter1/)もあるので、気になる方はそちらも参照してみてください。
~~わからなかったらLLMに聞いてみよう~~
