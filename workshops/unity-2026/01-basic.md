...
* この記事は Unity 講習会 2026 の資料です.
* Unity やってみたいけど何をすればいいのかわからない人を0を1にするための講習会です.
* 2025年度版から時間の都合で分量を削減したものです．
...
* **以下の手順は, 既にUnityとUnityが使える任意のエディタ(Rider, Visual Studio)が導入されている方は飛ばして構いません.

### 1.2.1 Unity Hubのダウンロード

[Unityの公式サイト](http://unity.com/ja) にアクセスし, Unity Hubのインストーラーをダウンロードします. 
![Unity Install](./unityInstall.png)

ログインを求められたら, 適当なアカウントで登録します. 

登録が完了したら,  `UnityHubSetup-x64.exe` (Macの人は `UnityHubSetup-x64.dmg`)をダウンロードし, インストール作業を完了します.

Unity Hubを起動したら, Sign Inを押して, 先ほどログインに使ったアカウントでログインします.

### 1.2.2 Unity Editorのダウンロード

Unity Hubの左側メニューの `Installs` をクリックし, 右上の `Install Editor` をクリックします. 
一覧の中から, `Unity 6.4 (6000.4.8f1)` を選択し, インストールを行います. 

`Add modules` は全てチェックを外してください.
時間かかるので, インストールの完了を待たずに次に進みます.

### 1.2.3 Riderのダウンロード

[Riderの公式サイト](https://www.jetbrains.com/rider/download/) にアクセスし, Riderのインストーラーをダウンロードします. 

インストーラーは起動に時間がかかるので, 何度もダブルクリックせず気長に待ちましょう.
